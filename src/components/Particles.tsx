...
export default function Particles({
  ...
  useEffect(() => {
    ...
    // WebGL may be unavailable (headless, blocklisted GPU, old browsers).
    // Fail gracefully instead of crashing the whole page.
    let renderer: Renderer;
    try {
      renderer = new Renderer({ dpr: pixelRatio, depth: false, alpha: true });
    } catch (e) {
      console.warn('Particles: WebGL unavailable, skipping background.', e);
      return;
    }
    ...
    const update = (t: number) => {
      ...
      if (!disableRotation) {
        ...
        (mesh.rotation as { z: number }).z += 0.01 * speed;
      }
